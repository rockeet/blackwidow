...
static void SetDelHistogram(size_t field_size, size_t value_size) {
...
Status RedisSets::Open(const BlackwidowOptions& bw_options,
  ...
  if (!bw_options.sideplugin_conf.empty()) {
    return OpenByRepo(bw_options, db_path, "sets");
  }
...
Status RedisSets::PKPatternMatchDel(const std::string& pattern,
  ...
  while (iter->Valid()) {
    ...
    if (!parsed_sets_meta_value.IsStale()
      ...
      SetKeyHistogram(length_histogram::Del, key.size());
      SetDelHistogram(0, parsed_sets_meta_value.user_value().size());
    }
...
Status RedisSets::SAdd(const Slice& key,
  ...
  if (s.ok()) {
    ...
    if (parsed_sets_meta_value.IsStale()
      ...
      batch.Put(handles_[0], key, meta_value);
      SetKeyHistogram(length_histogram::Add, key.size());
      for (const auto& member : filtered_members) {
        ...
        SetAddHistogram(0, sets_member_key.Encode().size());
      }
...
Status RedisSets::SDiffstore(const Slice& destination,
  ...
  if (s.ok()) {
    ParsedSetsMetaValue parsed_sets_meta_value(&meta_value);
    if (parsed_sets_meta_value.count() != 0) SetKeyHistogram(length_histogram::Del, destination.size());
    ...
    SetKeyHistogram(length_histogram::Add, destination.size());
  } else if (s.IsNotFound()) {
    char str[4];
    ...
    if (parsed_sets_meta_value.IsStale()) {
      ...
      for (iter->Seek(sets_member_key.Encode());
        ...
        if (cur_index == target_index) {
          ...
          SetDelHistogram(0, iter->key().size());
          ParsedSetsMemberKey parsed_sets_member_key(iter->key(), &parse_key_buf);
          *member = parsed_sets_member_key.member().ToString();

          parsed_sets_meta_value.ModifyCount(-1);
          batch.Put(handles_[0], key, meta_value);
          if (parsed_sets_meta_value.count() == 0) SetKeyHistogram(length_histogram::Del, key.size());
          break;
        }
      }
      delete iter;
