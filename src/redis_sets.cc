...
static void set_key_histogram(db_rw_histogram::process_type add_del, size_t size) {
  g_db_read_write_histogram->Add_Histogram_Metric(db_rw_histogram::Set, add_del, db_rw_histogram::Key, size);
}
  ...
  if (s.ok()) {
    ...
    if (parsed_sets_meta_value.IsStale()
      ...
      for (const auto& member : filtered_members) {
        ...
        set_add_histogram(0, sets_member_key.Encode().size());
      }
    ...
    if (parsed_sets_meta_value.IsStale()) {
      ...
      for (iter->Seek(sets_member_key.Encode());
        ...
        if (cur_index == target_index) {
          ...
          set_del_histogram(0, iter->key().size());
          ParsedSetsMemberKey parsed_sets_member_key(iter->key(), &parse_key_buf);
    ...
    if (parsed_sets_meta_value.IsStale()) {
      ...
      for (const auto& member : members) {
        ...
        if (s.ok()) {
          ...
          statistic++;
          batch.Delete(handles_[1], sets_member_key.Encode());
          set_del_histogram(0, member.size());
        } else if (s.IsNotFound()) {
        } else {
          return s;
        }
